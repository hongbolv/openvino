...
OpenVINO 的核心推理流程分为两个阶段：**初始化阶段**（一次性执行）和**推理执行阶段**（可对多张图像重复执行）。

```
┌─────────────── 初始化阶段（一次性） ───────────────┐
│ 模型读取 → 预处理配置 → 模型编译 → 推理请求创建    │
│ (Core)     (PrePostProcessor)  (Core)  (CompiledModel)│
└──────────────────────────────────────────────────────┘
                        ↓
┌─────────── 推理执行阶段（每张图像重复） ────────────┐
│ 设置输入张量 → 执行推理 → 获取输出张量              │
...
// ============================================================
// 初始化阶段（一次性）— 模型读取、预处理配置、编译
// 以下操作只需在程序启动时执行一次，后续可对任意数量的图像复用
// ============================================================
...
// Step 4: 编译模型到设备（耗时操作，包含图优化和 kernel 生成）
ov::CompiledModel compiled_model = core.compile_model(model, "CPU");

// Step 5: 创建推理请求（分配推理所需的内部缓冲区）
...
for (const auto& image_path : image_list) {
    ...
    ov::Tensor input_tensor(ov::element::u8, {1, img.rows, img.cols, 3}, img.data);
    ...
    // 输出形状: {1, 3, img.rows*4, img.cols*4}，值域 [0,1]
...
**设计理由：** 将推理过程明确拆分为初始化阶段和执行阶段，体现了 OpenVINO 的核心设计原则：

- **初始化阶段（一次性开销）**：模型读取、预处理配置和模型编译是计算密集型的一次性操作。特别是 `compile_model()` 会执行图优化变换、算子融合和设备特定的 kernel 生成，耗时较长（秒级）。预处理配置在编译前融合到计算图中，使得运行时无需额外的预处理开销。这些操作在整个程序生命周期中只需执行一次。
- **执行阶段（可重复）**：设置输入张量、执行推理和获取输出是轻量级操作，可以对多张图像序列重复执行。`InferRequest` 对象可复用，只需更新输入数据即可。Real-ESRGAN 模型的输入输出均为图像张量，因此后处理阶段需将 float32 输出反归一化为像素值并保存为高分辨率图像文件。
...
int main(int argc, char* argv[]) {
    try {
        ...
        // 3e) 构建预处理管线，将转换步骤插入模型计算图
        model = ppp.build();

        // -------- Step 4. 编译模型到目标设备 --------
        // compile_model() 执行：图优化 → 算子融合 → 设备特定 kernel 映射
        // 这是最耗时的初始化操作，编译结果可复用于所有后续推理
        ov::CompiledModel compiled_model = core.compile_model(model, device_name);

        // -------- Step 5. 创建推理请求 --------
        // InferRequest 可在多次推理间复用，只需更新输入数据
        ov::InferRequest infer_request = compiled_model.create_infer_request();

        // ====================================================================
        // 推理执行阶段（可重复）— 以下操作可对每张图像重复执行
        // 在处理多张图像序列时，只需从此处循环，无需重新加载或编译模型
        // ====================================================================
...
Real-ESRGAN 推理展示了 OpenVINO C++ API 在超分辨率场景下的典型使用模式，明确区分初始化阶段与推理执行阶段：

```
┌─── 初始化阶段（一次性） ───────────────────────────┐
│ Core 初始化  →  ONNX 模型加载                       │
│   ↓                                                  │
│ PrePostProcessor 配置  →  u8→f32 + NHWC→NCHW + scale │
│   ↓                                                  │
│ 模型编译到设备  →  创建推理请求                       │
└──────────────────────────────────────────────────────┘
                      ↓
...
│ 同步推理  →  输出张量获取                             │
│   ↓                                                  │
│ 后处理: 反归一化 + NCHW→NHWC + clamp  →  保存图像    │
└──────────────────────────────────────────────────────┘
```

**关键设计决策：**

1. **初始化与执行分离**：模型读取、预处理配置和编译是计算密集的一次性操作（秒级耗时），完成后可对任意数量的图像复用 `InferRequest` 对象，只需更新输入数据即可；
2. **零拷贝输入**：直接包装 `cv::Mat` 内存作为 `ov::Tensor`，避免图像数据拷贝，在高分辨率场景下尤为重要；
3. **PrePostProcessor 融合**：将归一化（`scale(255.0f)`）和布局转换（NHWC→NCHW）嵌入计算图，编译时可被优化器融合到后续计算中，使推理执行阶段无额外预处理开销；
4. **同步推理**：超分辨率任务通常单图处理且计算量大，同步模式简化了代码逻辑；异步模式可用于分块推理场景以重叠 I/O 和计算。

**代码组织：**

Real-ESRGAN 推理代码遵循 OpenVINO 标准模板，初始化阶段（Steps 1-5：Core 初始化 → 模型加载 → 预处理 → 编译 → 创建请求）只执行一次，推理执行阶段（Steps 6-9：图像读取 → 设置输入 → 推理 → 后处理）可对多张图像循环执行。配合编号注释和阶段标注，便于读者快速区分一次性初始化代码和可复用的推理循环。
